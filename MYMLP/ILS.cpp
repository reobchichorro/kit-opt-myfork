...
void ILS::UpdateAllSubseq(Solution *s, vector<vector<Subsequence>>& subseq_matrix) {
    ...
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++)
            subseq_matrix[i][j].Concatenate(subseq_matrix[i][j-1], subseq_matrix[j][j],  t[subseq_matrix[i][j-1].last][subseq_matrix[j][j].first]);
